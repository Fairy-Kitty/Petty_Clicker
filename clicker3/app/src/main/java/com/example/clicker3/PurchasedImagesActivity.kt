...
enum class PurchasedScreen { MAIN, SHOP, COLLECTION }
...
fun PurchasedImagesScreen(
    ...
    var nextScreen by remember { mutableStateOf<PurchasedScreen?>(null) }
    ...
    if (isLoading) {
        ...
        LaunchedEffect(nextScreen) {
            delay(300)
            when (nextScreen) {
                PurchasedScreen.MAIN -> onNavigateToMain()
                PurchasedScreen.SHOP -> onNavigateToShop()
                PurchasedScreen.COLLECTION -> { /* ничего не делать, уже на коллекции */ }
                else -> {}
            }
            isLoading = false
            nextScreen = null
    ...
    BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
        ...
        Box(
            modifier = Modifier
                ...
                .clickable {
                    nextScreen = PurchasedScreen.SHOP
                    isLoading = true
                }
        ) {
            Image(
        ...
        Box(
            modifier = Modifier
                ...
                .clickable {
                    nextScreen = PurchasedScreen.COLLECTION
                    isLoading = true
                }
        ) {
            Image(
        ...
        Box(
            modifier = Modifier
                ...
                .clickable {
                    nextScreen = PurchasedScreen.MAIN
                    isLoading = true
                }
        ) {
            Image(
